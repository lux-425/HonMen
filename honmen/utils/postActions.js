...
export const deletePost = async (postId, setPosts, setShowToastr) => {
  try {
    ...
    setPosts((prev) => prev.filter((post) => post._id !== postId)); // 大切
    setShowToastr(true);
  } catch (error) {
...
export const likePost = async (postId, userId, setLikes, like = true) => {
  try {
    if (like) {
      ...
      setLikes((prev) => prev.filter((like) => like.user !== userId)); // 大切
    }
  } catch (error) {
    alert(catchErrors(error));
  }
...
export const postComment = async (postId, user, text, setComments, setText) => {
  try {
    ...
    setComments((prev) => [newComment, ...prev]); // 再大切
    setText('');
  } catch (error) {
    alert(catchErrors(error));
  }
...
export const deleteComment = async (postId, commentId, setComments) => {
  try {
    ...
    setComments((prev) => prev.filter((comment) => comment._id !== commentId)); // 再大切
  } catch (error) {
    alert(catchErrors(error));
  }
};
